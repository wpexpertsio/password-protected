...
class Password_Protected {
	...
	public function allow_ip_addresses( $bool ) {
		...
		if ( in_array( $user_ip_address, $ip_addresses ) ) {
			$bool = false;
		}
	...
	public function get_user_ip_address() {
		...
		if ( isset( $_SERVER['REMOTE_ADDR'] ) ) {
			...
			if ( function_exists( 'ipv4_in_range' ) && strpos( $_SERVER['REMOTE_ADDR'], ':' ) === false ) {
			...
			} elseif ( function_exists( 'get_ipv6_full' ) && function_exists( 'ipv6_in_range' ) ) {
